...
*/
export const fetchAssistantResponse = async (runId: string, threadId: string, setStatusMessage: (message: string) => void, setProgress: (progress: number) => void, initialProgress: number): Promise<{message: string, history: any[]}> => {
  try {
    if(runId){
    ...
    return { message: response.messages, history: response.historyMessages };
  } catch (error) {
